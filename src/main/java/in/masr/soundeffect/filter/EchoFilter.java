package in.masr.soundeffect.filter;
...
public class EchoFilter extends SoundFilter {
	 ...
	 * specified decay rate.
	 * <p>
	 * The number of delay second specifies how long before the echo is
	 * initially heard.
	 * <p>
	 * The decay value is how much the echo has decayed from the source. A decay
	 * value of .5 means the echo heard is half as loud as the source.
	 * <p>
	 * The format is the AudioFormat of the stream being processed. It is used
	 * for calculating the buffer size.
	 */
	public EchoFilter(float delaySec, float decay, AudioFormat format) {
		int buffereSize = Math.round(format.getFrameRate() * delaySec
				* format.getFrameSize() / (Short.SIZE / Byte.SIZE));
		delayBuffer = new short[buffereSize];
		this.decay = decay;
	}
	...
	public int getRemainingSize() {
		...
		// derived from Math.pow(decay,numRemainingBuffers) <= finalDecay
	...
	public void filter(byte[] samples, int offset, int length) {
		...
		for (int i = offset; i < offset + length; i += 2) {
			...
			short newSample = 0;
			int newSampleIntVal = (int) (oldSample + decay
					* delayBuffer[delayBufferPos]);
			if (newSampleIntVal > Short.MAX_VALUE) {
				newSample = Short.MAX_VALUE;
			} else if (newSampleIntVal < Short.MIN_VALUE) {
				newSample = Short.MIN_VALUE;
			} else {
				newSample = (short) newSampleIntVal;
			}
